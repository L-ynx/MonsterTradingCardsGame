...
public class CardRepository extends Repository {
    ...
    public List<Card> showCards(String username, String table) {
        ...
        if (table.equals("cards"))
            query = "SELECT card_id, name, damage FROM " + table + " WHERE username = ?";
        else
            query = "SELECT cards.card_id, cards.name, cards.damage FROM cards JOIN decks " +
        ...
        try (Connection connection = dbConnection.getConnection()) {
            ...
            try (PreparedStatement stmt = connection.prepareStatement(query)){
                ...
                while (result.next()) {
                    ...
                    System.out.println("DIS DA COUNT: "+count);
                    if (count == 4)
                        return true;
                }
            } finally {
